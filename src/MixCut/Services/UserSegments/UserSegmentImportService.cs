...
public sealed class UserSegmentImportService
{
    ...
    /// </summary>
    /// <param name="onStage">处理阶段回调（"识别中"/"打标中"），供 UI 显示占位进度。</param>
        ...
        Action<string>? onStage = null, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            catch (Exception ex)
            {
                ...
                return new(name, UserSegmentImportStatus.SkippedUnreadable, null, null, "无法读取该文件");
            }
            if (dur <= 0) return new(name, UserSegmentImportStatus.SkippedUnreadable, null, null, "无法读取时长");
            if (dur > MaxDurationSec)
                return new(name, UserSegmentImportStatus.SkippedTooLong, null, null, $"{dur:F1}s 超过 15 秒");

            // 2) hash + 全局去重（同 hash 视频已存在 → 跳过，复用现有导入去重语义）。
            var hash = ImportViewModel.ComputeFileHash(filePath);
            await using (var dbDup = await _dbFactory.CreateDbContextAsync(ct))
            {
                if (hash is not null && await dbDup.Videos.AnyAsync(v => v.ContentHash == hash, ct))
                    return new(name, UserSegmentImportStatus.SkippedDuplicate, null, null, "已存在，已跳过");
            }
